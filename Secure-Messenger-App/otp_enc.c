 ...
 * 
 * Description: Takes in a plaintext message, key and a port number
 * 	to communicate with a server. If communication is allowed,
 * 	the server will return a ciphertext using one-time pad
 * 	encryption. Before sending plaintext and key to the server,
 * 	this program will verify the message only contains valid
 * 	message characters. Finally, if run to completion, the
...
int main(int argc, char *argv[])
{
  ...
  // Filestream for reading in the plaintext and key files
  FILE* file;

  // Lengths & sizes of the key and plaintext files
  int len_key, len_plain; 
  size_t size,size_plain=0,size_key=0; // zero for dynamic getline

  // Socket File Descriptor Number and Port Number
  int socketFD, port; 
  ...
  char* buff_key = NULL;   // uses getline so deallocate
  char* buff_plain = NULL; // uses getline so deallocate
  char* buffer = NULL;
  char* host = "localhost"; 

  // Authorization Message for allowed communication 
  char* msg_auth = "trogdor*";     // Request permission to send
  char* msg_roger = "burninator";  // Request to send granted
  int msg_size = 0;
  ...
  // get length of the plaintext (buff_plain)
  ...
  // Make sure Plaintext Message ONLY contains valid characters
  if(!validMessage(buff_plain))
  {
    free(buff_plain);
    fprintf(stderr,"otp_enc Error: %s input contains bad characters\n", argv[1]);
    exit(1);
  }
  ...
  // Create One Big Character Array to store [plaintext+key*] 
  // strcat the two buffers (plaintext and key) into 1 single message to send
  ...
  if(msg_size <0)
  {
    ...
    fprintf(stderr,"Error: could not receive message from otp_enc_d on port %d\n",port);
    exit(2);
  }
  ...
  {
    // Send the Combined Plaintext+Key Message stored in buffer
    sendMessage(socketFD, buffer);

    free(buffer);
    buffer = NULL;

    // Read in the returned encrypted text from client
    ...
    // Check if the Server Experienced any Errors while Encrypting
    char* err = NULL;
